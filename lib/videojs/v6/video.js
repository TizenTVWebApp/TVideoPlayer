...
var ProgressControl = function (_Component) {
  ...
  function ProgressControl(player, options) {
    ...
    _this.seekBarTimer = null;
    _this.seekBarPoint = null;
  ...
  ProgressControl.prototype.handleMouseMove = function handleMouseMove(event) {
	  console.log("ProgressControl handleMouseMove");
    var seekBar = this.getChild('seekBar');
    var mouseTimeDisplay = seekBar.getChild('mouseTimeDisplay');
    var seekBarEl = seekBar.el();
    var seekBarRect = getBoundingClientRect(seekBarEl);
    this.seekBarPoint = getPointerPosition(seekBarEl, event).x;

    // The default skin has a gap on either side of the `SeekBar`. This means
    // that it's possible to trigger this behavior outside the boundaries of
    // the `SeekBar`. This ensures we stay within it at all times.
    if (this.seekBarPoint > 1) {
    	this.seekBarPoint = 1;
    } else if (this.seekBarPoint < 0) {
    	this.seekBarPoint = 0;
    }

    if (mouseTimeDisplay) {
      mouseTimeDisplay.update(seekBarRect, this.seekBarPoint);
    }
  };
  
  ProgressControl.prototype.handleSeekingMove = function handleSeekingMove(direction) {
	  console.log("ProgressControl handleSeekingMove"+direction);
	  this.addClass('hover');
    var seekBar = this.getChild('seekBar');
    seekBar.addClass('hover');
    var mouseTimeDisplay = seekBar.getChild('mouseTimeDisplay');
    var seekBarEl = seekBar.el();
    var seekBarRect = getBoundingClientRect(seekBarEl);
    if(this.seekBarTimer){
    	this.clearTimeout(this.seekBarTimer);
    }    
       
    if(this.seekBarPoint==null){
    	this.seekBarPoint = this.player_.currentTime()/this.player_.duration();
    }
    
    this.seekBarPoint += direction?0.01:-0.01;   
    ...
    this.seekBarTimer = this.setTimeout(function(){
    	console.log("seekto"+this.seekBarPoint*this.player_.duration());
    	this.player_.currentTime(this.seekBarPoint*this.player_.duration());
    	this.seekBarPoint =null;
    	this.clearTimeout(this.seekBarTimer);
    	this.removeClass('hover');
    	this.getChild('seekBar').removeClass('hover');
    },2000);
  };
  ...
  ProgressControl.prototype.handleMouseSeek = function handleMouseSeek(event) {
	  console.log("ProgressControl handleMouseSeek");
    var seekBar = this.getChild('seekBar');
